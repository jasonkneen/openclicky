...
final class WikiViewerPanelManager {
    ...
    func show(index: WikiManager.Index, sourceRootURL: URL?) {
        if panel == nil {
            panel = makePanel(index: index, sourceRootURL: sourceRootURL)
        } else {
            updatePanel(index: index, sourceRootURL: sourceRootURL)
        }
    ...
    private func updatePanel(index: WikiManager.Index, sourceRootURL: URL?) {
        guard let hostingView = panel?.contentView as? NSHostingView<ClickyMemoryWindowView> else {
            return
        }

        hostingView.rootView = ClickyMemoryWindowView(index: index, sourceRootURL: sourceRootURL)
    }

    private func makePanel(index: WikiManager.Index, sourceRootURL: URL?) -> NSWindow {
        let hostingView = NSHostingView(rootView: ClickyMemoryWindowView(index: index, sourceRootURL: sourceRootURL))
        let window = NSWindow(
